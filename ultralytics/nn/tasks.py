...
class BaseModel(torch.nn.Module):
    ...
    def _apply(self, fn):
        ...
        if isinstance(
            m, (Detect, Detect_ConvNeXt, Detect_ExtraDW, Detect_IB)
...
class DetectionModel(BaseModel):
    ...
    def __init__(self, cfg="yolo11n.yaml", ch=3, nc=None, verbose=True):
        ...
        # Build strides
        m = self.model[-1]  # Detect()
        if isinstance(m, (Detect, Detect_ConvNeXt, Detect_ExtraDW, Detect_IB)):  # includes all Detect subclasses like Segment, Pose, OBB, YOLOEDetect, YOLOESegment
            s = 256  # 2x min stride
...
def parse_model(d, ch, verbose=True):
    ...
    for i, (f, n, m, args) in enumerate(d["backbone"] + d["head"]):  # from, number, module, args
        ...
        if m in base_modules:
            ...
            {Detect, Detect_ConvNeXt, Detect_ExtraDW, Detect_IB, WorldDetect, YOLOEDetect, Segment, YOLOESegment, Pose, OBB, ImagePoolingAttn, v10Detect}
        ):
            args.append([ch[x] for x in f])
            if m is Segment or m is YOLOESegment:
                args[2] = make_divisible(min(args[2], max_channels) * width, 8)
            if m in {Detect, Detect_ConvNeXt, Detect_ExtraDW, Detect_IB, YOLOEDetect, Segment, YOLOESegment, Pose, OBB}:
                m.legacy = legacy
    ...
    if isinstance(model, torch.nn.Module):  # PyTorch model
        ...
        for m in model.modules():
            if isinstance(m, (Segment, YOLOESegment)):
                ...
                return "obb"
            elif isinstance(m, (Detect, WorldDetect, YOLOEDetect, v10Detect, Detect_ConvNeXt, Detect_ExtraDW, Detect_IB)):
                return "detect"
