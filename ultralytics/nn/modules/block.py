...
class ConvNeXt(nn.Module):
    """Standard ConvNeXt"""
    def __init__(
        self, c1: int, c2: int, shortcut: bool = True, g: int = 1, k: Tuple[int, int] = (1, 1), e: float = 1.0, init_layer_scale=1e-3
    ):
        super().__init__()
        assert c1 == c2, "Cannot assign ConvNeXt with different in/out channels"
        assert g == 1, "Do not allow grouped convolution"
...
class ConvNeXtSequence(nn.Module):
    """Faster Implementation of CSP Bottleneck with 2 convolutions."""
...
class C3NX2(nn.Module):
    ...
    def __init__(
        self, c1: int, c2: int, n: int = 1, c3nx: bool = False, e: float = 0.5, g: int = 1, shortcut: bool = True
    ):
        """
        Initialize C3k2 module.
        ...
        self.cv2 = Conv((2 + n) * self.c, c2, 1)  # optional act=FReLU(c2)
        self.m = nn.ModuleList(
            C3NX(self.c, self.c, 2, shortcut, g) if c3nx else ConvNeXt(self.c, self.c, shortcut, g, e=4.0) for _ in range(n)
        )
...
class C3NX2_WoSilu(nn.Module):
    """Faster Implementation of CSP Bottleneck with 2 convolutions."""
    """C3NX2 with C3NX_WoSilu"""
    def __init__(
        self, c1: int, c2: int, n: int = 1, c3nx: bool = False, e: float = 0.5, g: int = 1, shortcut: bool = True
    ):
        """
        ...
        self.cv2 = Conv((2 + n) * self.c, c2, 1)  # optional act=FReLU(c2)
        self.m = nn.ModuleList(
            C3NX_WoSilu(self.c, self.c, 2, shortcut, g) if c3nx else ConvNeXt(self.c, self.c, shortcut, g, e=4.0) for _ in range(n)
        )
...
class C3NX(nn.Module):
    """C3NX is a CSP bottleneck module with customizable kernel sizes for feature extraction in neural networks."""
...
class C3NX_WoSilu(nn.Module):
    """C3NX is a CSP bottleneck module with customizable kernel sizes for feature extraction in neural networks."""
    """delete the silu of last conv"""
    def __init__(self, c1: int, c2: int, n: int = 1, shortcut: bool = True, g: int = 1, e: float = 1.0):
        """
        Initialize C3NX module.
        ...
        c_ = int(c2 * e)  # hidden channels
        self.cv1 = Conv(c1, c_, 1, 1)
        self.cv2 = Conv(c_, c2, 1, act=False)  # optional act=FReLU(c2)
        self.m = nn.Sequential(*(ConvNeXt(c_, c_, shortcut, g, e=2.0) for _ in range(n)))
...
class HourglassInvertBottleneck(nn.Module):
...
class HourglassConvNeXt(nn.Module):
    ...
    def __init__(self, c1: int, c2: int, n: int = 1, e: List[float] = [1.0], g: int = 1, shortcut: bool = True, init_layer_scale=1e-3):
        """
    ...
    def forward(self, x: torch.Tensor) -> torch.Tensor:
        """Forward pass through the CSP bottleneck with 3 convolutions."""
        return self.act(self.ls * self.cv2((self.m(self.cv1(x))))) + x    

class HourglassConvNeXt_Depth(nn.Module):
    """add 3*3 DW before eveny 1*1 conv in hourglassConvNeXt block"""
    ...
    def __init__(self, c1: int, c2: int, n: int = 1, e: List[float] = [1.0], g: int = 1, shortcut: bool = True):
        ...
        if len(e) > 1:
            self.m = nn.Sequential(*(HourglassConvNeXt_Depth(c_, c_, 2, e[1:], g, shortcut) for _ in range(n)))
        else:
            self.m = nn.Sequential(*(ConvNeXt(c_, c_, shortcut, g, e=2.0) for _ in range(n)))
...
class HourglassConvNeXt_WoSilu(nn.Module):
    """An hourglass bottleneck module with customizable expansion ratios for feature extraction in neural networks."""

    def __init__(self, c1: int, c2: int, n: int = 1, e: List[float] = [1.0], g: int = 1, shortcut: bool = True, has_Silu: bool = True):
        """
        Initialize HourglassInvertBottleneck module.
        ...
        if has_Silu:
            ...
            self.cv2 = Conv(c_, c2, 1, act=False)
        if len(e) > 1:
            self.m = nn.Sequential(*(HourglassConvNeXt_WoSilu(c_, c_, 2, e[1:], g, shortcut, has_Silu=False) for _ in range(n)))
        else:
            self.m = nn.Sequential(*(ConvNeXt(c_, c_, shortcut, g, e=2.0) for _ in range(n)))
...
class UpsampleMerge(nn.Module):
    ...
    def __init__(self, c1: int, c2: int, c3: int, act=True):
        ...
        """
        super().__init__()
        self.cv1 = ConvTranspose(c1, c3, act=False)
