...
from .block import DFL, SAVPE, BNContrastiveHead, ContrastiveHead, Proto, Residual, SwiGLUFFN, ConvNeXt, MobileInvertBottleneck, ExtraDepthWise
from .conv import Conv, DWConv
from .transformer import MLP, DeformableTransformerDecoder, DeformableTransformerDecoderLayer
from .utils import bias_init_with_prob, linear_init

__all__ = "Detect", "Detect_ConvNeXt", "Detect_IB", "Detect_ExtraDW", "Segment", "Pose", "Classify", "OBB", "RTDETRDecoder", "v10Detect", "YOLOEDetect", "YOLOESegment"


class Detect_ConvNeXt(nn.Module):
    """
    YOLO Detect head for object detection models, for cv2, use ConvNeXt to replace two dense conv
    ...
    def __init__(self, nc: int = 80, ch: Tuple = ()):
        ...
        self.cv2 = nn.ModuleList(
            nn.Sequential(Conv(x, c2, 1), ConvNeXt(c2, c2, e=4.0), nn.Conv2d(c2, 4 * self.reg_max, 1)) for x in ch
        )
        self.cv3 = (
...
class Detect_IB(nn.Module):
    """
    YOLO Detect head for object detection models, for cv2, use MobileInvertBottleneck to replace two dense conv
    ...
    def __init__(self, nc: int = 80, ch: Tuple = ()):
        ...
        c2, c3 = max((16, ch[0] // 4, self.reg_max * 4)), max(ch[0], min(self.nc, 100))  # channels
        self.cv2 = nn.ModuleList(
            nn.Sequential(Conv(x, c2, 1), MobileInvertBottleneck(c1=c2, c2=c2, e=4.0), nn.Conv2d(c2, 4 * self.reg_max, 1)) for x in ch
        )
        self.cv3 = (
            nn.ModuleList(nn.Sequential(Conv(x, c3, 3), Conv(c3, c3, 3), nn.Conv2d(c3, self.nc, 1)) for x in ch)
            if self.legacy
...
class Detect_ExtraDW(nn.Module):
    """
    YOLO Detect head for object detection models, for cv2, use ConvNeXt to replace two dense conv
    ...
    def __init__(self, nc: int = 80, ch: Tuple = ()):
        ...
        c2, c3 = max((16, ch[0] // 4, self.reg_max * 4)), max(ch[0], min(self.nc, 100))  # channels
        self.cv2 = nn.ModuleList(
            nn.Sequential(Conv(x, c2, 1), ExtraDepthWise(c1=c2, c2=c2, e=4.0), nn.Conv2d(c2, 4 * self.reg_max, 1)) for x in ch
        )
        self.cv3 = (
            nn.ModuleList(nn.Sequential(Conv(x, c3, 3), Conv(c3, c3, 3), nn.Conv2d(c3, self.nc, 1)) for x in ch)
            if self.legacy
